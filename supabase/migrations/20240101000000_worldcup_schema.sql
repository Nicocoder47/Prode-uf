...
-- Perfiles vinculados a auth.users (debe existir antes de funciones/triggers)
...
CREATE POLICY teams_admin ON teams FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY matches_admin ON matches FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY token_transactions_admin ON token_transactions FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY leaderboard_admin ON leaderboard FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY events_admin ON events FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());

ALTER TABLE audit_logs ENABLE ROW LEVEL SECURITY;
CREATE POLICY audit_logs_admin ON audit_logs FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY player_market_values_admin ON player_market_values FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY player_availability_admin ON player_availability FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY player_ratings_admin ON player_ratings FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());

ALTER TABLE lineups ENABLE ROW LEVEL SECURITY;
CREATE POLICY lineups_select ON lineups FOR SELECT USING (auth.role() = 'authenticated' OR auth.uid() IS NOT NULL);
CREATE POLICY lineups_admin ON lineups FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());

ALTER TABLE data_sync_logs ENABLE ROW LEVEL SECURITY;
CREATE POLICY data_sync_logs_admin ON data_sync_logs FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());

ALTER TABLE provider_mappings ENABLE ROW LEVEL SECURITY;
CREATE POLICY provider_mappings_admin ON provider_mappings FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());
