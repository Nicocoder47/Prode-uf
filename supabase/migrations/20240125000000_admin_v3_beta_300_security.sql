-- Admin V3 + Beta 300 security (idempotente)
...
WHERE password_changed_at IS NULL AND created_at < now() - interval '1 minute';

-- ---------------------------------------------------------------------------
-- 3. device_reports (si no existe)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. Helper: contar admins activos
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5. validate_registration — respetar invitaciones cerradas
-- ---------------------------------------------------------------------------
...
-- sync_user_profile: nuevos perfiles heredan must_change_password=true por DEFAULT de columna.

-- ---------------------------------------------------------------------------
-- 6. complete_password_change
-- ---------------------------------------------------------------------------
...
-- save_prediction: guard password_change_required vía migración 250 patch (no reemplazar core scoring).

-- ---------------------------------------------------------------------------
-- 7. RPCs admin — invitaciones
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 10. admin_soft_delete_user — endurecido
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 11. admin_block_user / admin_unblock_user
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  UPDATE public.profiles SET
    is_active = false, is_blocked = true,
    blocked_at = now(), blocked_by = v_actor,
    block_reason = nullif(trim(coalesce(p_reason, '')), ''),
    updated_at = now()
  WHERE id = p_user_id;

  PERFORM public.insert_activity_log(p_user_id, v_actor, 'user_blocked', 'Usuario bloqueado', p_reason, '{}'::jsonb);
  RETURN jsonb_build_object('ok', true);
END;
$$;
...
BEGIN
  ...
  PERFORM public.insert_activity_log(p_user_id, v_actor, 'user_unblocked', 'Usuario desbloqueado', NULL, '{}'::jsonb);
  RETURN jsonb_build_object('ok', true);
END;
$$;

-- ---------------------------------------------------------------------------
-- 12. admin_force_password_change
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 13. admin_update_user_role (reemplaza admin_set_user_role con validaciones)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 14. admin_set_notification_active
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  UPDATE public.notifications SET is_active = p_active, updated_at = now()
...
-- ---------------------------------------------------------------------------
-- 15. admin_get_users — campos password + actividad
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 16. admin_get_beta_capacity — registration_open + p95 fields
-- ---------------------------------------------------------------------------
...
AS $$
DECLARE
  v_registered int; v_new_today int; v_new_7d int; v_new_30d int;
  v_active_24h int; v_active_7d int; v_total_preds int; v_pred_24h int; v_pred_7d int;
  v_users_with_pred int; v_sync_errors int; v_last_sync jsonb;
  v_estimated int; v_capacity_pct numeric; v_status text; v_recommendation text;
  v_migration_needed boolean; v_reasons jsonb := '[]'::jsonb;
  v_device_health jsonb; v_reports_24h int; v_errors_24h int; v_error_rate numeric;
  v_read_p95 numeric; v_save_p95 numeric; v_auth_429 int;
  v_reg_open boolean;
  v_must_change int; v_blocked int; v_deactivated int;
BEGIN
  IF NOT public.is_admin() THEN RAISE EXCEPTION 'forbidden'; END IF;
  ...
  v_estimated := greatest(round(v_active_24h * 0.4)::int, round(v_active_7d * 0.35)::int, round(v_registered * 0.12)::int, 5);
  v_capacity_pct := least(100, round((v_registered::numeric / 300) * 1000) / 10);

  IF v_registered > 300 THEN
    v_status := 'exceeded'; v_recommendation := 'Excede beta 300. No permitir nuevos registros.'; v_migration_needed := true;
    v_reasons := v_reasons || jsonb_build_array('Usuarios superan límite 300');
  ELSIF v_registered >= 280 THEN
    v_status := 'red'; v_recommendation := 'Crítico: cerrar invitaciones y preparar migración.'; v_migration_needed := true;
    v_reasons := v_reasons || jsonb_build_array('Zona crítica ≥ 280 usuarios');
  ELSIF v_registered >= 220 THEN
    v_status := 'yellow'; v_recommendation := 'Atención: acercándose al límite de beta 300.'; v_migration_needed := false;
    v_reasons := v_reasons || jsonb_build_array('≥ 220 usuarios registrados');
  ELSE
    v_status := 'green'; v_recommendation := 'Beta saludable. Puede seguir gratis.'; v_migration_needed := false;
  ...
  v_error_rate := CASE WHEN v_reports_24h > 0 THEN round((v_errors_24h::numeric / v_reports_24h) * 1000) / 10 ELSE 0 END;
  ...
  v_device_health := jsonb_build_object(
    'reports_24h', v_reports_24h, 'errors_24h', v_errors_24h, 'error_rate', v_error_rate,
    'errors_by_device', (SELECT coalesce(jsonb_object_agg(device_type, cnt), '{}'::jsonb) FROM (
      SELECT device_type, count(*)::int cnt FROM public.device_reports
      WHERE event_type = 'error' AND created_at >= now() - interval '24 hours' GROUP BY device_type) d),
    'errors_by_browser', (SELECT coalesce(jsonb_object_agg(browser, cnt), '{}'::jsonb) FROM (
      SELECT coalesce(browser, 'unknown') browser, count(*)::int cnt FROM public.device_reports
      WHERE event_type = 'error' AND created_at >= now() - interval '24 hours' GROUP BY browser) b),
    'slow_routes', (SELECT coalesce(jsonb_agg(row_to_json(s)), '[]'::jsonb) FROM (
      SELECT route, round(avg(performance_ms))::int avg_ms FROM public.device_reports
      WHERE event_type = 'performance' AND performance_ms IS NOT NULL AND created_at >= now() - interval '24 hours'
      GROUP BY route ORDER BY avg(performance_ms) DESC LIMIT 5) s),
    'top_error_routes', (SELECT coalesce(jsonb_agg(row_to_json(s)), '[]'::jsonb) FROM (
      SELECT route, count(*)::int count FROM public.device_reports
      WHERE event_type = 'error' AND created_at >= now() - interval '24 hours'
      GROUP BY route ORDER BY count(*) DESC LIMIT 5) s),
    'mobile_error_share_pct', (SELECT CASE WHEN v_errors_24h > 0 THEN
      round(((SELECT count(*)::numeric FROM public.device_reports WHERE event_type = 'error' AND device_type = 'mobile'
        AND created_at >= now() - interval '24 hours') / v_errors_24h) * 1000) / 10 ELSE 0 END)
  );

  RETURN jsonb_build_object(
    'registered_users', v_registered, 'new_users_today', v_new_today, 'new_users_7d', v_new_7d, 'new_users_30d', v_new_30d,
    'active_users_24h', v_active_24h, 'active_users_7d', v_active_7d,
    'total_predictions', v_total_preds, 'predictions_24h', v_pred_24h, 'predictions_7d', v_pred_7d,
    'users_with_predictions', v_users_with_pred,
    'users_played_pct', CASE WHEN v_registered > 0 THEN round((v_users_with_pred::numeric / v_registered) * 100, 1) ELSE 0 END,
    'estimated_concurrent_users', v_estimated, 'capacity_percent', v_capacity_pct,
    'status', v_status, 'recommendation', v_recommendation,
    'technical_action', CASE
      ...
      ELSE 'seguir_gratis' END,
    'registration_open', v_reg_open,
    'migration_needed', v_migration_needed, 'reasons', v_reasons,
    'recent_sync_errors_24h', v_sync_errors, 'auth_errors_429_24h', v_auth_429,
    'read_p95_ms', v_read_p95, 'save_p95_ms', v_save_p95,
    'users_must_change_password', v_must_change, 'users_blocked', v_blocked, 'users_deactivated', v_deactivated,
    'device_health', v_device_health, 'last_sync', v_last_sync,
    'latest_snapshots', (SELECT coalesce(jsonb_agg(row_to_json(s) ORDER BY s.created_at DESC), '[]'::jsonb) FROM (
      SELECT id, created_at, total_users, active_users_7d, total_predictions, estimated_concurrent_peak,
             capacity_status, migration_recommendation, notes
      FROM public.system_capacity_snapshots ORDER BY created_at DESC LIMIT 10) s),
