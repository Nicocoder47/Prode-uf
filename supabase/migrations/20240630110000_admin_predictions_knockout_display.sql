-- Admin detalle: mostrar alargue y ganador por penales en predicciones knockout
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      pred.predicted_et_score_home,
      pred.predicted_et_score_away,
      ...
      m.score_home_after_et AS result_home_after_et,
      m.score_away_after_et AS result_away_after_et,
      CASE
        WHEN m.score_home_after_et IS NOT NULL AND m.score_home IS NOT NULL
        THEN m.score_home_after_et - m.score_home
        ELSE NULL
      END AS result_et_home,
      CASE
        WHEN m.score_away_after_et IS NOT NULL AND m.score_away IS NOT NULL
        THEN m.score_away_after_et - m.score_away
        ELSE NULL
      END AS result_et_away,
      m.score_home_penalties AS result_home_penalties,
      m.score_away_penalties AS result_away_penalties,
      CASE
        WHEN m.score_home_penalties IS NOT NULL AND m.score_away_penalties IS NOT NULL
             AND m.score_home_penalties > m.score_away_penalties THEN 'home'
        WHEN m.score_home_penalties IS NOT NULL AND m.score_away_penalties IS NOT NULL
             AND m.score_away_penalties > m.score_home_penalties THEN 'away'
        ELSE NULL
      END AS result_penalty_winner,
