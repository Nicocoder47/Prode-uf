-- Eliminatorias: predicción de alargue acumulado y ganador por penales.
-- Puntaje knockout: 5 (90') + 3 (tras alargue) + 2 (penales) = 10 máx.

DROP FUNCTION IF EXISTS public.save_prediction(uuid, integer, integer);

ALTER TABLE matches
  ADD COLUMN IF NOT EXISTS score_home_after_et integer,
  ADD COLUMN IF NOT EXISTS score_away_after_et integer;

ALTER TABLE predictions
  ADD COLUMN IF NOT EXISTS predicted_et_score_home integer,
  ADD COLUMN IF NOT EXISTS predicted_et_score_away integer,
  ADD COLUMN IF NOT EXISTS predicted_penalty_winner text;

COMMENT ON COLUMN matches.score_home_after_et IS 'Marcador acumulado al finalizar el alargue (incluye 90 min).';
COMMENT ON COLUMN predictions.predicted_et_score_home IS 'Marcador acumulado predicho tras alargue (obligatorio si empate en 90 min en eliminatoria).';
COMMENT ON COLUMN predictions.predicted_penalty_winner IS 'home | away — clasificado por penales si empate tras alargue.';

CREATE OR REPLACE FUNCTION public.is_knockout_match(p_group_label text, p_phase text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT
    p_group_label IS NULL
    AND coalesce(trim(p_phase), '') <> ''
    AND upper(trim(p_phase)) NOT IN ('GROUP', 'GROUP_STAGE')
    AND upper(trim(p_phase)) NOT LIKE 'GROUP%';
$$;

CREATE OR REPLACE FUNCTION public.penalty_winner_from_match(
  p_home_pen integer,
  p_away_pen integer
)
RETURNS text
...
AS $$
  SELECT CASE
    WHEN p_home_pen IS NULL OR p_away_pen IS NULL OR p_home_pen = p_away_pen THEN NULL
    WHEN p_home_pen > p_away_pen THEN 'home'
    ELSE 'away'
  END;
$$;
...
BEGIN
  ...
  v_match_had_et :=
    ...
    AND (
      ...
      OR public.penalty_winner_from_match(p_match.score_home_penalties, p_match.score_away_penalties) IS NOT NULL
    );
  ...
  IF v_knockout THEN
    ...
    IF v_match_had_et
       ...
       AND p_pred.predicted_et_score_home = p_match.score_home_after_et
       AND p_pred.predicted_et_score_away = p_match.score_away_after_et THEN
      v_points := v_points + 3;
    END IF;
...
BEGIN
  ...
  IF v_knockout AND v_winner = 'draw' THEN
    ...
    END IF;
    IF p_et_score_home < p_score_home OR p_et_score_away < p_score_away THEN
      RAISE EXCEPTION 'knockout_et_invalid';
    END IF;
