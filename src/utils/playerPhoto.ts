/** photoUrl (API) → photo (dominio) → fallbacks por ID externo */
...
export function resolvePlayerPhoto(player: PlayerPhotoInput): string | null {
  return player.photoUrl ?? player.photo ?? null;
}

export function buildPlayerPhotoCandidates(player: PlayerPhotoInput): string[] {
  const urls: string[] = [];
  const primary = resolvePlayerPhoto(player);
  if (primary) urls.push(primary);

  const apiId = player.apiFootballId
    ?? (player.provider === 'api_football' ? player.providerPlayerId : null);
  if (apiId) {
    urls.push(`https://media.api-sports.io/football/players/${apiId}.png`);
  }

  if (player.theSportsDbId) {
    urls.push(`https://www.thesportsdb.com/images/media/player/cutout/${player.theSportsDbId}.png`);
    urls.push(`https://www.thesportsdb.com/images/media/player/thumb/${player.theSportsDbId}.jpg`);
  }
