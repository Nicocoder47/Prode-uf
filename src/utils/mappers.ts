...
export function mapDbMatchToMatch(dbMatch: Record<string, unknown>): Match {
  ...
  return {
    ...
    homeScoreAfterEt: (dbMatch.score_home_after_et as number | null) ?? null,
    awayScoreAfterEt: (dbMatch.score_away_after_et as number | null) ?? null,
    homePenalties: (dbMatch.score_home_penalties as number | null) ?? null,
    awayPenalties: (dbMatch.score_away_penalties as number | null) ?? null,
...
export function mapDbPredictionToPrediction(dbPred: Record<string, unknown>): Prediction {
  ...
  return {
    ...
    predictedEtHomeScore: (dbPred.predicted_et_score_home as number | null) ?? null,
    predictedEtAwayScore: (dbPred.predicted_et_score_away as number | null) ?? null,
    predictedPenaltyWinner: (dbPred.predicted_penalty_winner as 'home' | 'away' | null) ?? null,
